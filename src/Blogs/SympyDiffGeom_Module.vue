<template>
  <div class="flex font-merriweather">
  <!-- Content -->
    <div class="w-[60vw] h-screen p-4 border-x-2 border-green-300 bg-grid">
      <header>
        <h1 class="text-4xl font-bold text-green-600 font-raleway text-center mt-4 mb-0.5">Sympy PDE Module</h1>
        <p class="text-sm text-black font-raleway text-right">January 2025</p>
        <div class="w-[98%] h-0.5 bg-gray-300 mx-auto mb-4"></div>
      </header>
      <main>
        <div></div>
      </main>
      <footer></footer>
    </div>
    <!-- Minimap -->
    <div class="h-screen w-[20vw] sticky top-0 bg-gray-100"></div>
  </div>
</template>

<style scoped>
.bg-grid {
    background-image: linear-gradient(to right, #6d6b6b11 1px, transparent 1px), 
                      linear-gradient(to bottom, #6d6b6b11 1px, transparent 1px);
    background-size: 6px 6px; 
}</style>